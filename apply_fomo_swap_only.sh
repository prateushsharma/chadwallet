...
# ChadWallet - swap panel reduced to ONLY fomo's Buy/Sell card (verbatim fomo markup).
#  Removes everything that was around it (the About card, Your positions, and all helper components);
#  the panel is now exactly fomo's card: toggle, amount + live estimate, $10/$100/$500/$1000 + settings,
#  '$X available', action button (fomo disabled style; enabled green/red when an amount is entered),
#  fee row (#tag 'Lowest fees: 0.05%' + #info). Wired to the existing Jupiter quote + paper execute.
# Run from repo root:  bash apply_fomo_swap_only.sh
...
cat > "src/components/trade/SwapPanel.tsx" << 'CHADFOMO_EOF'
"use client";
...
git add -- "src/components/trade/SwapPanel.tsx"
if git diff --cached --quiet; then echo "no changes"; else git commit -q -m "feat(trade): swap panel is now only the fomo Buy/Sell card (strip About + positions)"; echo "commit ok"; fi
rm -rf .next
