...
# ChadWallet — fomo's right-click chart context menu, built from fomo's markup + icons, WIRED.
#  Chart runs on our own BirdEye renderer (lightweight-charts) inside the fomo shell so the custom menu
#  can be injected (a cross-origin TradingView iframe can't be). The contextmenu is captured on the pane
#  in the CAPTURE phase so it fires before lightweight-charts' canvas swallows it (that was the bug).
#  Right-click the chart -> Reset chart view (Alt+R), Copy price, Paste, Lock vertical cursor line by time,
#  Table view, Remove 1 drawing, Remove 1 indicator, Hide marks on bars, Settings…
#  Wired: Reset->fit, Copy price->clipboard, Lock cursor->toggle vertical crosshair, Table view->OHLCV
#  popover, Remove indicator->toggle volume, Settings->log scale. (Paste/Remove drawing/Hide marks close.)
#  NOTE: if you're currently on the TradingView widget, this switches the chart to our renderer (the widget
#  iframe cannot host a custom menu). TradingViewChart.tsx stays on disk for the pure-widget look.
# Run from repo root:  bash apply_fomo_chart_menu.sh
...
grep -q '"lightweight-charts"' package.json || { echo "ERROR: lightweight-charts missing from deps."; exit 1; }

mkdir -p "src/components/trade"
cat > "src/components/trade/PriceChart.tsx" << 'CHADFOMO_EOF'
"use client";
...
export function ChartArea({ token }: { token: Token }) {
  ...
  const [table, setTable] = useState<{ time: number; open: number; high: number; low: number; close: number; volume: number }[] | null>(null);
  const [noVert, setNoVert] = useState(false);
  ...
  useEffect(() => { if (!menu) return; const c = () => setMenu(null); const k = (e: KeyboardEvent) => { if (e.key === "Escape") setMenu(null); }; window.addEventListener("click", c); window.addEventListener("keydown", k); return () => { window.removeEventListener("click", c); window.removeEventListener("keydown", k); }; }, [menu]);
  useEffect(() => { const k = (e: KeyboardEvent) => { if (e.altKey && (e.key === "r" || e.key === "R")) { e.preventDefault(); chart.current?.fitContent(); } }; window.addEventListener("keydown", k); return () => window.removeEventListener("keydown", k); }, []);
  ...
  return (
    <div ref={wrap} className="flex flex-col bg-bg-primary">
      ...
      <div className="flex h-[380px]">
        ...
        {/* chart pane */}
        <div ref={pane} className="group relative flex-1">
          ...
          {/* right-click context menu — fomo / TradingView */}
          {menu && (
            <div className="absolute z-modal min-w-[248px] rounded-lg border border-bg-tertiary bg-bg-secondary py-1 text-sm text-text-primary shadow-2xl"
              style={{ left: Math.min(menu.x, (pane.current?.clientWidth ?? 400) - 256), top: Math.min(menu.y, (pane.current?.clientHeight ?? 300) - 60) }}
              onClick={(e) => e.stopPropagation()}>
              <MI icon={ResetIcon} label="Reset chart view" shortcut="Alt + R" onClick={() => { chart.current?.fitContent(); setMenu(null); }} />
              <MSep />
              <MI label={`Copy price ${leg ? lp(leg.c) : ""}`} onClick={() => { if (leg) navigator.clipboard?.writeText(lp(leg.c)); setMenu(null); }} />
              <MI label="Paste" shortcut="Ctrl + V" onClick={() => setMenu(null)} />
              <MSep />
              <MI label="Lock vertical cursor line by time" check={noVert} onClick={() => { const n = !noVert; setNoVert(n); chart.current?.setCrosshairVert(!n); setMenu(null); }} />
              <MSep />
              <MI label="Table view" onClick={() => { setTable((chart.current?.getCandles() ?? []).slice(-40).reverse()); setMenu(null); }} />
              <MSep />
              <MI label="Remove 1 drawing" onClick={() => setMenu(null)} />
              <MI label="Remove 1 indicator" onClick={() => { chart.current?.toggleVolume(); setMenu(null); }} />
              <MSep />
              <MI label="Hide marks on bars" onClick={() => setMenu(null)} />
              <MSep />
              <MI icon={GearIcon} label="Settings…" onClick={() => { setMode((m) => (m === 1 ? 0 : 1)); setMenu(null); }} />
            </div>
          )}

          {/* table view popover */}
          {table && (
            <div className="absolute right-2 top-2 z-modal max-h-[320px] w-[330px] overflow-auto rounded-lg border border-bg-tertiary bg-bg-secondary text-xs shadow-2xl" onClick={(e) => e.stopPropagation()}>
              <div className="sticky top-0 flex items-center justify-between border-b border-bg-tertiary bg-bg-secondary px-3 py-2 text-text-primary">
                <span className="font-medium">Table view · {token.symbol}</span>
                <button type="button" onClick={() => setTable(null)} className="text-text-tertiary hover:text-text-primary">✕</button>
              </div>
              <table className="w-full tabular-nums">
                <thead><tr className="text-text-tertiary">
                  <th className="px-2 py-1 text-left font-normal">Time</th><th className="px-2 py-1 text-right font-normal">O</th><th className="px-2 py-1 text-right font-normal">H</th><th className="px-2 py-1 text-right font-normal">L</th><th className="px-2 py-1 text-right font-normal">C</th><th className="px-2 py-1 text-right font-normal">Vol</th>
                </tr></thead>
                <tbody>
                  {table.map((c) => (
                    <tr key={c.time} className={c.close >= c.open ? "text-green" : "text-red"}>
                      <td className="px-2 py-0.5 text-left text-text-secondary">{new Date(c.time * 1000).toUTCString().slice(17, 22)}</td>
                      <td className="px-2 py-0.5 text-right">{lp(c.open)}</td>
                      <td className="px-2 py-0.5 text-right">{lp(c.high)}</td>
                      <td className="px-2 py-0.5 text-right">{lp(c.low)}</td>
                      <td className="px-2 py-0.5 text-right">{lp(c.close)}</td>
                      <td className="px-2 py-0.5 text-right text-text-secondary">{lv(c.volume)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
function MI({ icon, label, shortcut, check, onClick }: { icon?: React.ReactNode; label: string; shortcut?: string; check?: boolean; onClick: () => void }) {
  return (
    <button type="button" onClick={onClick} className="flex h-[34px] w-full items-center gap-2 px-2 text-left hover:bg-bg-tertiary/60">
      <span className="flex h-7 w-7 shrink-0 items-center justify-center text-text-secondary">
        {check ? <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true"><path d="M5 12l5 5L20 6" strokeLinecap="round" strokeLinejoin="round" /></svg> : icon}
      </span>
      <span className="flex-1 whitespace-nowrap">{label}</span>
      {shortcut && <span className="ml-6 whitespace-nowrap text-text-tertiary">{shortcut}</span>}
    </button>
  );
}
function MSep() { return <div className="my-1 h-px bg-bg-tertiary" />; }
const ResetIcon = (
  <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 28 28" aria-hidden="true"><g fill="none" fillRule="evenodd" stroke="currentColor"><path d="M6.5 15A8.5 8.5 0 1 0 15 6.5H8.5" /><path d="M12 10L8.5 6.5 12 3" /></g></svg>
);
const GearIcon = (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 28 28" width="20" height="20" fill="currentColor" aria-hidden="true"><path fillRule="evenodd" d="M18 14a4 4 0 1 1-8 0 4 4 0 0 1 8 0Zm-1 0a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" /><path fillRule="evenodd" d="M8.5 5h11l5 9-5 9h-11l-5-9 5-9Zm-3.86 9L9.1 6h9.82l4.45 8-4.45 8H9.1l-4.45-8Z" /></svg>
);
CHADFOMO_EOF

git add -- "src/components/trade/PriceChart.tsx" "src/components/trade/ChartArea.tsx"
if git diff --cached --quiet; then echo "· no changes"; else git commit -q -m "fix(trade): capture-phase contextmenu so fomo right-click menu actually opens over the chart"; echo "commit ok"; fi
rm -rf .next
echo; echo "Applied. FULLY restart:  pkill -f \"next dev\"; rm -rf .next; npm run dev   then hard-refresh /trade and right-click the chart."
