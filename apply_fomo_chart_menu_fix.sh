...
# ChadWallet - fix: right-click menu broke the other chart buttons.
#  The capture-phase contextmenu handler called stopPropagation(), which stopped the event from reaching
#  lightweight-charts' canvas and left its pointer state out of sync, so it began ignoring later pointer
#  events (killing the toolbar/zoom buttons). Now it only calls preventDefault() (suppress the browser's
#  native menu) and lets the event through, so the chart stays responsive AND the right-click menu still opens.
# Run from repo root:  bash apply_fomo_chart_menu_fix.sh
...
git add -- "src/components/trade/ChartArea.tsx"
if git diff --cached --quiet; then echo "no changes"; else git commit -q -m "fix(trade): dont stopPropagation on chart contextmenu (kept toolbar/zoom buttons responsive)"; echo "commit ok"; fi
rm -rf .next
echo; echo "Applied. Restart:  pkill -f \"next dev\"; rm -rf .next; npm run dev   then hard-refresh /trade."
